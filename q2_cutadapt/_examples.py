...
#    sequence_url = ('https://data.qiime2.org/usage-examples/cutadapt/'
#                    'paired_multiplexed.qza')
    sequence_url = 'https://qiime2-data.s3.us-west-2.amazonaws.com/usage-examples/other/paired_multiplexed.qza'
    metadata_url = ('https://data.qiime2.org/usage-examples/other/paired_metadata.tsv')
 
    seqs = use.init_artifact_from_url('seqs', sequence_url)
    md = use.init_metadata_from_url('md', metadata_url)
    forward_barcodes = use.get_metadata_column('barcodes', 'barcode-sequence', md)
    #reverse_barcodes = use.get_metadata_column('barcodes', 'barcode-sequence', md)
    ...
    per_sample_sequences.assert_output_type('SampleData[PairedEndSequencesWithQuality]')
    untrimmed_sequences.assert_output_type(
