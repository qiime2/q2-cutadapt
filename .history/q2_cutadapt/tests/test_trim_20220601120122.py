...
class TestTrimUtilsSingle(TestPluginBase):
    ...
    def test_build_trim_command_typical(self):
        ...
        for _, fwd in df.itertuples():
            ...
            self.assertTrue('--max_expected_errors 1' in obs)
            self.assertTrue('--quality_cutoff_3end 20' in obs)
            self.assertTrue('--quality_cutoff_5end 20' in obs)
            self.assertTrue('--quality_base 33' in obs)
